...
#update dictionary to with flag_freq of a window
def update_flag_freq_by_window_dict(flag_freq_by_window_dict, flag_freq):

    if flag_freq not in flag_freq_by_window_dict:
        flag_freq_by_window_dict[flag_freq] = 0

    flag_freq_by_window_dict[flag_freq] += 1
...
def determine_chromosome_window(chr, read_pos):
    ...
    return str(chr) + "_" + str(window)

#update flag dictionary to count one more deletion
def increment_deletions(flag_dict):

    chromosome_window = determine_chromosome_window(chromosome, read_pos)
...
sam_file_path = sys.argv[3]
output_path = sys.argv[4]
del_dup_bam_dir_path = sys.argv[5]
...
del_flag_freq_by_window = {}
dupl_flag_freq_by_window = {}
...
for window in flag_dict:
    ...
    #maintain dictionaries of flag frequencies per window for deletions and duplications
    if del_count > 0:
        del_flag_freq_by_window = update_flag_freq_by_window_dict(del_flag_freq_by_window, del_count)
    
    if dup_count > 0:
        dupl_flag_freq_by_window = update_flag_freq_by_window_dict(dupl_flag_freq_by_window, dup_count)
...
#iterate through dictionaries and write flag freq by window for deletions and duplications to files
del_flag_freq_output_path = working_dir + sample_name + "_del_flag_freq.txt"
dup_flag_freq_output_path = working_dir + sample_name + "_dup_flag_freq.txt"

del_flag_freq_output_file = open(del_flag_freq_output_path, "w")
dup_flag_freq_output_file = open(dup_flag_freq_output_path, "w")

del_flag_freq_output_file.write("Flag Freq\tNumber of Windows\n")
dup_flag_freq_output_file.write("Flag Freq\tNumber of Windows\n")

for freq in del_flag_freq_by_window:
    window_count = del_flag_freq_by_window[freq]
    del_flag_freq_output_file.write(str(freq) + "\t" + str(window_count) + "\n")

for freq in dupl_flag_freq_by_window:
    window_count = dupl_flag_freq_by_window[freq]
    dup_flag_freq_output_file.write(str(freq) + "\t" + str(window_count) + "\n")

del_flag_freq_output_file.close()
dup_flag_freq_output_file.close()

#if del_max >= 5 or dup_max >= 5:
#if del_max >= 3 or dup_max >= 3:
if del_max >= 0 or dup_max >= 0:
